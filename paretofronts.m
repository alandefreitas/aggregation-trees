function [M,F] = paretofronts(x,objective,dom,make_plot,parameter,normalization)
...
%                           7 - 'lorenz'
%
%    make_plot = 0 (no plot), 1 (plot with all fronts when nargout == 2), 
%                 or 2 (plot with the 1st front)
%
...
%
%   normalization = normalizes (if true) the objective values in the 
%               parallel coordinates if there are more than 3 objectives
%
%
% Outputs:
%    M = list with which elements are in the first front
%    F = list with the front of each element
%    (The function with nargout == 1 is much more efficient)
%
% Examples: 
% 
...
if length(objective) == 1
    objective = ones(1,size(x,2))*objective;
end
...
if (make_plot>0)
    ...
    if size(x,2) == 2
        ...
        hold off;
        % if the user wants to plot the dominated points
        if (make_plot > 1)
            % if we defined a second output we have information on the fronts
            % and are going to color the points accordingly
            if nargout > 1
                for i=max(F):-1:2
                    plot(x((F==i),1),x((F==i),2),'.','color',[0 ((-i+max(F))/(max(F)-min(F))) 1-0.4*((-i+max(F))/(max(F)-min(F)))]);
                    hold on;
                end
            % if there is less than 2 outputs, we don't know all the fronts and
    ...
    % if the problem is 3-D and there is no normalization
    elseif (size(x,2) == 3)&&(nargin<6)
        for i=1:size(x,2)
            if objective(i)==0
                x(:,i) = -x(:,i);
            end
        ...
        % For any other number of dimensions (or 3-D with normalization)
        for i=1:size(x,2)
            if objective(i)==0
                x(:,i) = -x(:,i);
            end
        end
        % normalizes the objective values from min (0) to max (1)
        if normalization
            minimum = min(x);
            for i=1:size(x,1)
                x(i,:) = x(i,:) - minimum;
        ...
        hold off;
        % plots the dominated members
        plot(x(M==0,:)','--k');
        xlim([1,size(x,2)]);
        hold on;
        % plots gray lines vertically for each obj
        for i=1:size(x,2)
